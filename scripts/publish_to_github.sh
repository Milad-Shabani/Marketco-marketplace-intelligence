...
if [[ -z "${REMOTE_URL}" ]]; then
  ...
  echo "Example: $0 https://github.com/Milad-Shabani/marketco-marketplace-intelligence.git" >&2
  exit 1
fi
...
if git diff --cached --quiet; then
  ...
  echo "==> Committing..."
  git commit -m "MarketCo: marketplace operations + Dynamics 365-style CRM dataset, order forecasting, churn model, and Excel/HTML reporting"
fi
